...
├─ KNOWLEDGE ─── AI_SKILL_MAP · AI_PROGRESSION_PLAN · AI_LEARNING_RESOURCES · AI_7_SKILLS · AI_SKILLS_ACQUISITION · AI_TYPES · AI_CLAUDE_TOOL_SELECTOR · AI_CLAUDE_STACK_2026 · AI_CLAUDE_GLOSSARY · AI_RAG_DESIGN · AI_PROJECT_STRUCTURE · AI_ROLES_2026 · AI_CLAUDE_ENGINEER_ROADMAP · AI_DATA_BI · AI_SYSTEM_DESIGN · AI_PROCESS_INTELLIGENCE · AI_PROFICIENCIES_2026 · AI_LINEAR_ALGEBRA · AGENT_BLUEPRINT · CLOUD_ARCHITECT_SKILLS · KUBERNETES_101 · AI_MICROSERVICES · AI_DATA_VALIDATION   ידע אישי/לימודי (on-demand)
...
| 31 | `AI_DATA_VALIDATION.md` | Knowledge | ✅ פעיל | 6 טכניקות ולידציית נתונים (Type · Range · Mandatory · Duplicate · Format · Consistency) → איזה כשל כל אחת מונעת ב-ULease (מחיר כ-string שובר PMT · רכב ב-₪15 · VIN כפול מנפח מלאי וספירת עסקאות · feed↔CRM↔מימון לא מצליבים) ובאיזו שכבה היא חיה (Edge/Service/DB/Pre-RAG/Monitoring); "Bad data = bad decisions" = תזת ULease — שער האיכות שמגן על שכבת ההחלטה |
| 32 | `INVESTOR_RELATIONS.md` | Business | ✅ פעיל | חברה, cap table, גיוס 150K, תחזית ומעקב משקיעים |
| 33 | `CASES/ULEASE*.md` | Business | ✅ פעיל | תיק ULease 🎯 — מודל עסקי, תחזית, איפיון, מתודולוגיה, גיוס, playbooks, מנועי אקווזיציה (היצע + ביקוש), ביקורת ומחירון/SLA |
