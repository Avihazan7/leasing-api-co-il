...
## Active Modules
- [`OPERATING_SYSTEM.md`](./OPERATING_SYSTEM.md) v1.6.0 — Kernel: core doctrine, layered architecture, module registry (§3 + §3.1 working sets), the activation Boot Block, and the conflict-resolution hierarchy.
...
- [`DECISION_LOG.md`](./DECISION_LOG.md) v1.20.0 — Append-only decision log: 33 founding OS + ULease decisions with rationale and status.
...
- [`AI_CLAUDE_TOOL_SELECTOR.md`](./AI_CLAUDE_TOOL_SELECTOR.md) v1.2.0 — "Which Claude should you use?": a decision tree over 15 Claude tools (Chat · Code · Cowork · Skills · Excel · Artifacts…), recommended models + the Opus 4.8 effort lever, and the mapping to OS/ULease workflows.
...
- [`AI_DATA_BI.md`](./AI_DATA_BI.md) v1.1.0 — BI & data-modeling foundations (Power BI as the study case, 15 topics): the BI pipeline, ETL, ULease's star schema, DAX + time intelligence, visualization guide, interactivity (drill-through · Q&A visual) and row-level security — mapped to M9 (the subscriber data product) with the Power-BI-vs-custom decision framed for the Tech Lead.
