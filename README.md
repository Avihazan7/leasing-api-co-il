...
- [`AGENT_BLUEPRINT.md`](./AGENT_BLUEPRINT.md) v1.0.0 — **System-first doctrine.** הגשר בין ה-Docs OS ל-Agent Runtime (`stage-a/`): 8-module blueprint (Purpose → Prompt → LLM → Tools → Memory → Orchestration → UI → Evals), coverage map כן, שכבת Evals, ו-governance שנאכף בקוד. הופך Skill ל-agent production-grade.
