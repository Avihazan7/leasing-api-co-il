...
## Active Modules
- `OPERATING_SYSTEM.md` v1.12.0 — Kernel: עקרונות יסוד, ארכיטקטורת שכבות, רישום מודולים (§3 + §3.1 תשתית תפעולית), Boot Block והיררכיית הכרעה.
- `MEMORY.md` v1.1.0 — שכבת זיכרון: schema, כרטיס זהות **מלא**, Active Focus/Projects, העדפות ופרוטוקול קריאה/כתיבה.
- `DECISION_LOG.md` v1.43.0 — יומן החלטות append-only: 56 החלטות (OS + ULease), רציונל וסטטוס.
...
9. `AI_*` + `AGENT_BLUEPRINT` (SKILL_MAP · PROGRESSION_PLAN · LEARNING_RESOURCES · 7_SKILLS · SKILLS_ACQUISITION · TYPES · CLAUDE_TOOL_SELECTOR · CLAUDE_STACK_2026 · CLAUDE_GLOSSARY · RAG_DESIGN · PROJECT_STRUCTURE · ROLES_2026 · CLAUDE_ENGINEER_ROADMAP · DATA_BI · SYSTEM_DESIGN · PROCESS_INTELLIGENCE · PROFICIENCIES_2026 · LINEAR_ALGEBRA · AGENT_BLUEPRINT)  ← Knowledge (on-demand)
