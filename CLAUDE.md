...
- `DECISION_LOG.md` v1.24.0 — יומן החלטות append-only: 37 החלטות (OS + ULease), רציונל וסטטוס.
...
- `AI_CLAUDE_STACK_2026.md` v1.3.0 — Claude Stack 2026: 4 עמודי ה-cheat sheet (Cowork · Projects · Skills · Code), כלל הזהב, מיפוי ה-build התפעולי בריפו, סולם 7 הרמות (אתה ברמה 6/7), Agent Teams כמסלול prototype ל-Ultra·Master·Max + ה-Agent Extension Stack (Skills·MCP·Subagents·Hooks·Plugins).
...
- `AI_DATA_BI.md` v1.3.0 — יסודות BI ומידול נתונים (Power BI כמקרה לימוד, 20 נושאים): צינור BI, ETL, ה-star schema של ULease, DAX + Time Intelligence, ויזואליזציה, אינטראקטיביות, **RLS**, תפעול כשירות (התראות · מנויי דוחות · הרשאות) + מבחנים סטטיסטיים להחלטות — ממופה ל-M9 + הכרעת כלי ל-Tech Lead.
- `AI_SYSTEM_DESIGN.md` v1.1.0 — יסודות System Design: שער כניסה (Gateway·Proxy·LB), 6 סגנונות API, תורים (Idempotency·DLQ), JWT + מפת 21 הרכיבים (MVP/V1/V2) — ממופים לארכיטקטורת הפלטפורמה + צ'קליסט design review ל-Tech Lead.
