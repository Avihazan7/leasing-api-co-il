...
- `DECISION_LOG.md` v1.52.0 — יומן החלטות append-only: 65 החלטות (OS + ULease), רציונל וסטטוס.
...
- `AGENT_BLUEPRINT.md` v1.1.0 — בלופרינט סוכן AI (How to Build an AI Agent): 8 שלבים (Purpose → System Prompt → LLM → Tools → Memory → Orchestration → Interface → Testing) כשכבת ניווט שמאנדקסת את משפחת ה-`AI_*` (כל שלב → הבית הקנוני) + §9 טבלת ה-Ecosystem (Claude Agent SDK בלבד, D-022) + **§10 כללי העבודה (דוקטרינת Karpathy)** — המקור הקנוני שאליו מפנה `leasing-api/CLAUDE.md`, סוגר הפניה שבורה חוצת-ריפו + **§11 שנים-עשר מונחי ה-Agentic** (אינדקס מונח→בית-קנוני→ULease, אחות ל-`AI_CLAUDE_GLOSSARY`).
