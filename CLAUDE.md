...
- `DECISION_LOG.md` v1.20.0 — יומן החלטות append-only: 33 החלטות (OS + ULease), רציונל וסטטוס.
...
- `AI_DATA_BI.md` v1.1.0 — יסודות BI ומידול נתונים (Power BI כמקרה לימוד, 15 נושאים): צינור BI, ETL, ה-star schema של ULease, DAX + Time Intelligence, ויזואליזציה, אינטראקטיביות (Drill-through · Q&A) ו-**RLS** — ממופה ל-M9 + הכרעת כלי ל-Tech Lead.
