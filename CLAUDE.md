...
- `AI_DATA_BI.md` v1.6.0 — יסודות BI ומידול נתונים (Power BI כמקרה לימוד, 20 נושאים): צינור BI, ETL, ה-star schema של ULease, DAX + Time Intelligence + 16 הפונקציות הפיננסיות (PMT · IRR · פחת/ערך שייר — המתמטיקה של הליסינג), ויזואליזציה, אינטראקטיביות, **RLS**, תפעול כשירות (התראות · מנויי דוחות · הרשאות), BI לשיווק (§7: ROAS·CPA·CLV·Churn + דשבורדי ההשקה + §7.4 חמשת היתרונות) + מבחנים סטטיסטיים להחלטות — ממופה ל-M9 + הכרעת כלי ל-Tech Lead.
...
- `AI_PROFICIENCIES_2026.md` v1.0.0 — מיומנויות ה-AI החיוניות ל-2026: 10 מיומנויות-על ארגוניות (אינפוגרפיקת aiplanetx) + כרטיס-ניקוד בגרות (**6✅+4🟡/10**, אפס נקודות עיוורות) ומיפוי תלת-שכבתי ל-ULease — שדרה (RAG · אוטונומי · governance) · צמיחה (GEO · workflows) · תשתית (prompting · ecosystem); מנדט ה-Tech Lead = להפוך את 4 ה-🟡 ל-✅. מצטרף לחמשת מדדי-הבגרות החיצוניים (5/7 · 11/15 · 6/7 · 10✅+1🟡/12 · 6✅+4🟡/10).
...
- `AI_10_SKILLS.md` v1.0.0 — 10 מיומנויות AI לשליטה ב-2026 (אינפוגרפיקה tool-centric): כל מיומנות → הכלים הקנוניים → הבית ב-OS/ULease (Prompting→COMMAND_API · Automation→מנועי n8n · Agents→AGENT_BLUEPRINT+Ultra·Master·Max · RAG→RAG_DESIGN/pgvector · Data→DATA_BI · Vision/Voice→שכבת היכולות · GEO→DEMAND_PLAYBOOK · Ethics→Guardian/evals) + כרטיס-ניקוד 6✅+4🟡/10 (אותה צורה כמו AI_PROFICIENCIES_2026) + עיקרון ה-Claude-first (Agent SDK · pgvector · n8n כבר עונים על הכלים הגנריים). העדשה הכלי-מוכוונת המשלימה ל-AI_7_SKILLS (הרגלים) ו-AI_PROFICIENCIES_2026 (ארגוני).
- `KUBERNETES_101.md` v1.0.0 — קוברנטיס 101 (תזמור קונטיינרים): הבעיה ש-K8s פותר, מושגי הליבה (Cluster/Node/Pod), ארכיטקטורת Control Plane/Worker (API Server · etcd · Scheduler · kubelet · kube-proxy · CRI), Workloads (Deployment/StatefulSet/DaemonSet/Job), רשת (Service/Ingress), Config/Secrets, Scaling/Resilience (HPA/self-healing/rolling), Observability, Security (Namespaces/RBAC/NetworkPolicy) ו-DevOps (CI/CD/GitOps) — ממופים ל-ULease + §11 **ההכרעה מתי/אם K8s** (MVP על docker-compose, K8s = V1+) ודגל אנטי-over-engineering; ה-deep-dive של שכבת ה-Cloud-Native ב-`CLOUD_ARCHITECT_SKILLS` §4.
