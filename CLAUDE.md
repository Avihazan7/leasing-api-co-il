...
- `AI_ACQUISITION_FLYWHEEL.md` v1.0.0 — גלגל התנופה לרכישת לקוחות (n8n Full Autopilot): 18 צמתים ב-3 פאזות בלולאה סגורה — לידים ממומנים → לקוח → תוכן → תנועה אורגנית → הלקוח הבא; הטריגר החסר "עסקה סגורה → תוכן", AI Receptionist לערוץ הנכנס, גבולות ציות (inbound בלבד, HITL) ומיפוי לצד-הביקוש של ULease.
...
9. `AI_*` (SKILL_MAP · PROGRESSION_PLAN · LEARNING_RESOURCES · 7_SKILLS · SKILLS_ACQUISITION · TYPES · CLAUDE_TOOL_SELECTOR · CLAUDE_STACK_2026 · CLAUDE_GLOSSARY · RAG_DESIGN · PROJECT_STRUCTURE · ROLES_2026 · CLAUDE_ENGINEER_ROADMAP · DATA_BI · SYSTEM_DESIGN · PROCESS_INTELLIGENCE · ACQUISITION_FLYWHEEL)  ← Knowledge (on-demand)
