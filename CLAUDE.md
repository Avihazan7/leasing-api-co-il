...
- `AI_DATA_BI.md` v1.6.0 — יסודות BI ומידול נתונים (Power BI כמקרה לימוד, 20 נושאים): צינור BI, ETL, ה-star schema של ULease, DAX + Time Intelligence + 16 הפונקציות הפיננסיות (PMT · IRR · פחת/ערך שייר — המתמטיקה של הליסינג) + IF/ALL (נתח שוק), ויזואליזציה (כולל Scatter ל-ROAS ו-Treemap), אינטראקטיביות, **RLS**, תפעול כשירות (התראות · מנויי דוחות · הרשאות), BI לשיווק (§7: ROAS·CPA·CLV·Churn + דשבורדי ההשקה) + מבחנים סטטיסטיים להחלטות — ממופה ל-M9 + הכרעת כלי ל-Tech Lead.
