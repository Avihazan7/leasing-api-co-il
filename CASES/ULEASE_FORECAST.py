...
for i,(y,m) in enumerate(months):
    ...
    ad_rev   = ad_k[i]*K
    total_rev= deal_rev+lead_rev+sub_rev+ad_rev
    ...
    rows.append(dict(period=f"{HEB_MON[m]} {y%100:02d}", y=y, m=m,
        ...
        deal_rev=deal_rev, lead_rev=lead_rev, sub_rev=sub_rev, ad_rev=ad_rev,
...
with open(out_csv,"w",newline="",encoding="utf-8-sig") as f:
    ...
    w.writerow(["period","deals","leads","subscribers","pro","pro_max",
                "deal_rev","lead_rev","sub_rev","ad_rev","total_rev","opex","net_monthly","cum_cash","gmv"])
    for r in rows:
        w.writerow([r["period"],r["deals"],r["leads"],r["subs"],r["pro"],r["promax"],
            r["deal_rev"],r["lead_rev"],r["sub_rev"],r["ad_rev"],r["total_rev"],r["opex"],r["net"],r["cum"],r["gmv"]])

# ---------------- פלט: טבלת Markdown חודשית ----------------
f = lambda n: f"{n:,.0f}"
print('| חודש | עסקאות | לידים | מנויים | הכנ׳ עסקאות | הכנ׳ לידים | הכנ׳ מנויים | פרסום | סה"כ הכנסה | הוצאות | נטו | Cash מצטבר |')
print('|------|------:|-----:|-----:|----------:|---------:|----------:|-----:|---------:|------:|----:|----------:|')
for r in rows:
    print(f"| {r['period']} | {r['deals']} | {r['leads']} | {r['subs']} | {f(r['deal_rev'])} | {f(r['lead_rev'])} | {f(r['sub_rev'])} | {f(r['ad_rev'])} | {f(r['total_rev'])} | {f(r['opex'])} | {f(r['net'])} | {f(r['cum'])} |")
...
def agg(yr):
    ...
    return dict(deals=sum(r["deals"] for r in sel), rev=sum(r["total_rev"] for r in sel),
                ...
                gmv=sum(r["gmv"] for r in sel), subs=sel[-1]["subs"])
a26, a27 = agg(2026), agg(2027)
print("\n=== סיכום שנתי ===")
print(f"2026 (יוני–דצמ'): עסקאות {a26['deals']:,} | GMV {a26['gmv']:,.0f} | הכנסה {a26['rev']:,.0f} | הוצאות {a26['opex']:,.0f} | נטו {a26['net']:,.0f} | מנויים {a26['subs']}")
print(f"2027 (ינו'–דצמ'): עסקאות {a27['deals']:,} | GMV {a27['gmv']:,.0f} | הכנסה {a27['rev']:,.0f} | הוצאות {a27['opex']:,.0f} | נטו {a27['net']:,.0f} | מנויים {a27['subs']}")
d = rows[-1]
